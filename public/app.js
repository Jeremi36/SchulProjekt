(() => {
  ...
  function openModal(title, body, options = {}) {
    ...
    backdrop.innerHTML = `
      <section class="modal ${options.large ? 'large' : ''}" role="dialog" aria-modal="true" aria-label="${title}">
        <header class="modal-head">
          <h2>${title}</h2>
          <button class="close-modal" data-action="close-modal" aria-label="Schließen">${icon('close')}</button>
        </header>
  ...
  app.addEventListener('click', event => {
    ...
    else if (action === 'landing') { history.pushState({}, '', '/'); state.role = null; render(); }
    else if (action === 'toggle-sidebar') { state.sidebarOpen = !state.sidebarOpen; document.querySelector('.app-shell')?.classList.toggle('sidebar-open', state.sidebarOpen); }
    ...
    else if (action === 'mark-read') { closeModal(); toast('Der Elternbrief wurde als gelesen markiert.'); }
    else if (action === 'save-generic') { closeModal(); toast('Änderung wurde im Prototyp gespeichert.'); }
  ...
  document.addEventListener('click', event => {
    ...
    if (!event.target.closest('.dropdown') && !event.target.closest('[data-action="notifications"]') && !event.target.closest('[data-action="profile-menu"]')) document.querySelectorAll('.dropdown').forEach(d => d.remove());
  });
  ...
  function showExam(subject) {
    ...
    document.querySelector('[data-action="go-ai"]')?.addEventListener('click', () => { closeModal(); navigate(state.role,'ai'); }, { once:true });
  }
  ...
  function simulateUpload() {
    ...
    document.querySelector('[data-action="finish-upload"]')?.addEventListener('click', () => { closeModal(); toast('Hefteintrag hochgeladen und zur KI-Analyse vorgemerkt.'); }, { once:true });
  }
  ...
  function render() {
    ...
    if (!state.role) renderLanding(); else renderShell();
